...
def train_and_evaluate_catboost(X_train, y_train, X_valid, y_valid, best_params=None):
    """
    Обучает и оценивает CatBoost регрессионную модель.
...
def train_and_evaluate_random_forest(X_train, y_train, X_valid, y_valid):
    """
    Обучает и оценивает Random Forest регрессионную модель.
    Возвращает обученную модель.
    """
...
if __name__ == "__main__":
    ...
    # Обучение и оценка модели Random Forest Regressor
    ...
    # Анализ важности признаков для Random Forest Regressor
    print("\nАнализ важности признаков для Random Forest Regressor:")
    analyze_feature_importance(rf_best, X_scaled.columns, model_name='Random Forest Regressor')
    ...
    cat_r2 = r2_score(y_valid, cat_best.predict(X_valid))
    rf_r2 = r2_score(y_valid, rf_best.predict(X_valid))
    ...
    print(f"\nВыбор лучшей модели: {best_model_name} с R² Score: {max(cat_r2, rf_r2):.4f}")

    # Предсказание и сохранение результатов
    print("\nПредсказание на контрольной выборке и сохранение результатов...")
    try:
        predict_and_save(best_model, X_check_scaled, check_df, output_path='result_experiment_catboost_or_randforest_regres.csv')
    except ValueError as e:
